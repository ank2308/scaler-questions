...
public class Solution {
	...
	public static void main(String[] args) {
		...
		System.out.println(Arrays.toString(sln.solve(A)));
	}
	...
	public int[] solve(int[] A) {
		...
		int ansLimit = (int) Math.sqrt(A.length);		
		
		int [] ans = new int[ansLimit];
		int sqrt;
		for (int i = freq.length-1,j=0; i >0; i--) {
			if(freq[i]%2 !=0) {
				sqrt = (int) Math.sqrt(freq[i]);
				if( freq[i] == sqrt*sqrt ) {
					while(sqrt > 0) {
						ans[j] = i;
						j++;
